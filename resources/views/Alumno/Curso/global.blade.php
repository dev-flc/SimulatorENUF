...
<br>
 <!-- ini -->
@if ($pregunta->count())
  {!! Form::open(['route'=>'globalfinal','method'=>'POST','id'=>'examenfinal']) !!}
@foreach($pregunta as $pre)
    ...
    <input type="hidden" name="unidadid" value="{{ $unidad->CUR_id }}">
    <input type="hidden" name="nombre" value="{{ $unidad->CUR_nombre }}">
    <br>
    <input type="hidden" name="tiempoexamen" id="tiempoexamen" value="{{ $unidad->CUR_tiempo }}">
